-- ============================================================================
-- SOS EMERGENCY FEATURE - DATABASE CHANGES
-- Version: 1.0
-- Date: 2026-01-26
-- Author: Generated by Feature Analysis Workflow
-- ============================================================================
...
CREATE INDEX idx_emergency_contacts_active ON user_emergency_contacts (user_id) 
...
-- Create partitions
CREATE TABLE sos_events_y2026m01 PARTITION OF sos_events 
    FOR VALUES FROM ('2026-01-01') TO ('2026-02-01');
CREATE TABLE sos_events_y2026m02 PARTITION OF sos_events 
    FOR VALUES FROM ('2026-02-01') TO ('2026-03-01');
CREATE TABLE sos_events_default PARTITION OF sos_events DEFAULT;
...
CREATE INDEX idx_sos_events_cooldown ON sos_events (user_id, countdown_completed_at DESC) 
    WHERE status = 1;
...
-- Create partitions
CREATE TABLE sos_notifications_y2026m01 PARTITION OF sos_notifications 
    FOR VALUES FROM ('2026-01-01') TO ('2026-02-01');
CREATE TABLE sos_notifications_y2026m02 PARTITION OF sos_notifications 
    FOR VALUES FROM ('2026-02-01') TO ('2026-03-01');
CREATE TABLE sos_notifications_default PARTITION OF sos_notifications DEFAULT;

-- Indexes
CREATE INDEX idx_sos_notifications_event ON sos_notifications (event_id);
CREATE INDEX idx_sos_notifications_status ON sos_notifications (status) 
    WHERE status IN (0, 4);
CREATE INDEX idx_sos_notifications_retry ON sos_notifications (next_retry_at) 
    WHERE status = 4;
...
CREATE INDEX idx_sos_escalation_calls_pending ON sos_escalation_calls (status) 
    WHERE status IN (0, 1);
...
-- DROP TABLE IF EXISTS first_aid_content CASCADE;
-- DROP TABLE IF EXISTS sos_escalation_calls CASCADE;
-- DROP TABLE IF EXISTS sos_notifications CASCADE;
